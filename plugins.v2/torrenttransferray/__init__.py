...
class TorrentTransfer(_PluginBase):
    # 插件名称
    plugin_name = "自动转移做种（支持 qBittorrent 5）"
    # 插件描述
    plugin_desc = "定期转移下载器中的做种任务到另一个下载器。"
    # 插件图标
    plugin_icon = "seed.png"
    # 插件版本
    plugin_version = "1.7.1.1"
    # 插件作者
    plugin_author = "jxxghp,so1ve"
    # 作者主页
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._torrent_tags = (
                self._add_torrent_tags.strip().split(",")
                if self._add_torrent_tags
                else []
            )
    ...
    def __validate_config(self) -> bool:
        ...
        if self._fromtorrentpath and not Path(self._fromtorrentpath).exists():
            ...
            self.systemmessage.put(
                f"源下载器种子文件保存路径不存在：{self._fromtorrentpath}",
                title="自动转移做种",
            )
            return False
        if self._fromdownloader == self._todownloader:
            logger.error("源下载器和目的下载器不能相同")
            self.systemmessage.put("源下载器和目的下载器不能相同", title="自动转移做种")
            return False
        return True

    def __download(
        self, service: ServiceInfo, content: bytes, save_path: str
    ) -> Optional[str]:
        """
        添加下载任务
        """
    ...
    def transfer(self):
        ...
        from_downloader: Optional[Union[Qbittorrent, Transmission]] = (
            from_service.instance if from_service else None
        )
        to_service = self.service_info(self._todownloader)
        to_downloader: Optional[Union[Qbittorrent, Transmission]] = (
            to_service.instance if to_service else None
        )
        ...
        for torrent in torrents:
            ...
            if self._nopaths and save_path:
                ...
                for nopath in self._nopaths.split("\n"):
                    if os.path.normpath(save_path).startswith(os.path.normpath(nopath)):
                        logger.info(
                            f"种子 {hash_str} 保存路径 {save_path} 不需要转移，跳过 ..."
                        )
                        nopath_skip = True
                        break
                if nopath_skip:
            ...
            # 种子为无标签,则进行规范化
            is_torrent_labels_empty = (
                torrent_labels == [""] or torrent_labels == [] or torrent_labels is None
            )
            ...
            if self._includecategory:
                ...
                if torrent_category not in self._includecategory.split(","):
                    logger.info(
                        f"种子 {hash_str} 不含有转移分类 {self._includecategory}，跳过 ..."
                    )
                    continue
            # 根据设置决定是否转移无标签的种子
            if is_torrent_labels_empty:
                ...
                if self._nolabels:
                    ...
                    for label in self._nolabels.split(","):
                        if label in torrent_labels:
                            logger.info(
                                f"种子 {hash_str} 含有不转移标签 {label}，跳过 ..."
                            )
                            is_skip = True
                            break
                    if is_skip:
                ...
                if self._includelabels:
                    ...
                    for label in self._includelabels.split(","):
                        if label not in torrent_labels:
                            logger.info(
                                f"种子 {hash_str} 不含有转移标签 {label}，跳过 ..."
                            )
                            is_skip = True
                            break
                    if is_skip:
            ...
            trans_torrents.append(
                {"hash": hash_str, "save_path": save_path, "torrent": torrent}
            )
        ...
        if trans_torrents:
            ...
            for torrent_item in trans_torrents:
                # 检查种子文件是否存在
                torrent_file = (
                    Path(self._fromtorrentpath) / f"{torrent_item.get('hash')}.torrent"
                )
                ...
                # 查询hash值是否已经在目的下载器中
                torrent_info, _ = to_downloader.get_torrents(
                    ids=[torrent_item.get("hash")]
                )
                if torrent_info:
                    # 删除重复的源种子，不能删除文件！
                    if self._deleteduplicate:
                        logger.info(
                            f"删除重复的源下载器任务（不含文件）：{torrent_item.get('hash')} ..."
                        )
                        to_downloader.delete_torrents(
                            delete_file=False, ids=[torrent_item.get("hash")]
                        )
                        del_dup += 1
                    else:
                        logger.info(
                            f"{torrent_item.get('hash')} 已在目的下载器中，跳过 ..."
                        )
                        # 跳过计数
                        skip += 1
                    continue
                ...
                # 转换保存路径
                download_dir = self.__convert_save_path(
                    torrent_item.get("save_path"), self._frompath, self._topath
                )
                if not download_dir:
                ...
                if self.downloader_helper.is_downloader(
                    "qbittorrent", service=from_service
                ):
                    # 读取种子内容、解析种子文件
                    ...
                    if not main_announce:
                        logger.info(
                            f"{torrent_item.get('hash')} 未发现tracker信息，尝试补充tracker信息..."
                        )
                        # 读取fastresume文件
                        fastresume_file = (
                            Path(self._fromtorrentpath)
                            / f"{torrent_item.get('hash')}.fastresume"
                        )
                        ...
                        try:
                            ...
                            if (
                                ...
                                if (
                                    len(fastresume_trackers) > 1
                                    or len(fastresume_trackers[0]) > 1
                                ):
                                    torrent_main["announce-list"] = fastresume_trackers
                                # 替换种子文件路径
                                torrent_file = (
                                    settings.TEMP_PATH
                                    / f"{torrent_item.get('hash')}.torrent"
                                )
                                # 编码并保存到临时文件
                                torrent_file.write_bytes(bencode(torrent_main))
                        except Exception as err:
                            logger.error(
                                f"解析fastresume文件 {fastresume_file} 出错：{str(err)}"
                            )
                            fail += 1
                            continue
                ...
                logger.info(
                    f"添加转移做种任务到下载器 {to_service.name}：{torrent_file}"
                )
                ...
                if not download_id:
                    ...
                    # TR会自动校验，QB需要手动校验
                    if self.downloader_helper.is_downloader(
                        "qbittorrent", service=to_service
                    ):
                    ...
                    if self._deletesource:
                        logger.info(
                            f"删除源下载器任务（不含文件）：{torrent_item.get('hash')} ..."
                        )
                        from_downloader.delete_torrents(
                            delete_file=False, ids=[torrent_item.get("hash")]
                        )
    ...
    def check_recheck(self):
        ...
        if torrents:
            ...
            if can_seeding_torrents:
                ...
                logger.info("没有新的任务校验完成，将在下次个周期继续检查 ...")

        elif torrents is None:
            logger.info(
                f"下载器 {to_service.name} 查询校验任务失败，将在下次继续查询 ..."
            )
        else:
            logger.info(
                f"下载器 {to_service.name} 中没有需要检查的校验任务，清空待处理列表"
            )
            self._recheck_torrents[to_service.name] = []
    ...
    def __get_hash(torrent: Any, dl_type: str):
        ...
        try:
            return (
                torrent.get("hash") if dl_type == "qbittorrent" else torrent.hashString
            )
    ...
    def __get_label(torrent: Any, dl_type: str):
        ...
        try:
            return (
                [str(tag).strip() for tag in torrent.get("tags").split(",")]
                if dl_type == "qbittorrent"
                else torrent.labels or []
            )
        except Exception as e:
            print(str(e))
            return []
    ...
    def __get_category(torrent: Any, dl_type: str):
        ...
        """
        try:
            return torrent.get("category").strip() if dl_type == "qbittorrent" else ""
        except Exception as e:
            print(str(e))
            return ""
    ...
    def __can_seeding(torrent: Any, dl_type: str):
        ...
        try:
            return (
                (
                    torrent.get("state") == "pausedUP"
                    or torrent.get("state") == "stoppedUP"
                )
                if dl_type == "qbittorrent"
